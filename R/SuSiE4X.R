...
#' @param Z An n × q matrix of covariates. An intercept will be added into Z.
#' @param y An n-vector of responses.
#' @param strong A logical indicator of whether to consider strong heredity constraint (default = TRUE).
...
#' @param crossprodX The cross-product matrix X'X (default = NULL). If provided, SuSiE4X will directly use it for fine-mapping.
...
Z <- cbind(1, Z)
colnames(Z) <- c("intercept", nameZ)

if(strong==T){
run_iter_strong(X = X, Z = Z, y = y, crossprodX = crossprodX,
...
if(nrow(X)>1000|ncol(X)>100){
warning("Estimating under weak heredity constraint can be substantial time-consuming for large X.")
}
run_iter_weak(X = X, Z = Z, y = y, crossprodX = crossprodX,
                Lmain = Lmain, Linteraction = Linteraction,
                max.iter = max.iter, max.eps = max.eps, min.iter = min.iter,
                susie.iter = susie.iter, verbose = verbose,n_threads=n_threads,...)
}
