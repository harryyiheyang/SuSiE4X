Run_GGE_Select_ZIP <- function(X, Z, y,
                        family = mgcv::ziP(),
                        mgcv_model = NULL,
                        Lmain, Lint, Lenv,
  ...
  env_init <- initial_continuous_env(
    Z, y, Lenv, n_threads = n_threads, block_size = suff_block_size
  )
  fitZ <- env_init$fitZ
  ZCS <- env_init$ZCS
  ...
  for (iter in seq_len(max.iter)) {
    ...
    ZI_env <- zip_nuisance_design(n, XCS_refit, WCS_refit)
    ssZ <- weighted_projected_suffstats(
      X = Z, y = work$z, ZI = ZI_env, weights = work$weights,
      nuisance_precision = projection_penalty_precision(ZI_env, fitX, fitW, fitZ),
      n_threads = n_threads, ridge = ridge, block_size = suff_block_size
    )
    fitZ <- .fit_susie_stage(
      structural = list(
        XtX = ssZ$XtX, Xty = ssZ$Xty, yty = ssZ$yty,
        n = max(0.95 * n, work$n_eff), L = Lenv
      ),
    ...
    ssX <- weighted_projected_suffstats(
      ...
      nuisance_precision = projection_penalty_precision(ZI_main, fitX, fitW, fitZ),
      n_threads = n_threads, ridge = ridge, block_size = suff_block_size
    )
  ...
  EnvIndex <- safe_add_p(Identifying_EnvEffect(fitZ, colnames(Z)), G)
  ...
  AA$fitZ <- fitZ
  AA$environment_discoveries <- EnvIndex
