Run_GGE_CLM <- function(X, Z, y, clm_link,
  ...
  pred_final <- ocat_predictor_data(Z = Z, n = n)
  ...
  for (iter in seq_len(max.iter)) {
    ...
    Znui_main <- cbind(if (nZ > 0L) Z else NULL, WCS_refit)
    ...
    if (is.null(x_component$design)) {
      ...
      XCS_refit <- XCS
      {
        main_noncs <- build_noncs_refit_term(
          ...
          cor_design = if (nZ > 0L) Z else NULL
        )
      ...
      Znui_int <- cbind(if (nZ > 0L) Z else NULL, XCS_refit)
      ...
      w_noncs <- build_w_noncs_refit_term(
        ...
        XCS = XCS, Z = if (nZ > 0L) Z else NULL,
